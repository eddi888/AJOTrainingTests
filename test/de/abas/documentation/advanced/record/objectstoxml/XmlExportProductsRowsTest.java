...
public class XmlExportProductsRowsTest extends AbstractTest {

	private final XmlExportProductsRows instance = new XmlExportProductsRows();
	...
	public void runTest() {
		...
		try {
			...
			for (final Element record : document.getRootElement().getChild("RecordSet").getChildren()) {
				...
				assertThat("head element has 4 field elements", headFields.size(), is(4));
				...
				if (record.getChildren("Row").size() > 0) {
					for (final Element row : record.getChildren("Row")) {
						final List<Element> rowFields = row.getChildren();
						assertThat("row element has 4 field elements", rowFields.size(), is(4));
						for (final Element field : rowFields) {
							assertThat("field has attribute 'name'", field.getAttribute("name").getName(), is(not("")));
							assertThat("field has attribute 'name'",
									field.getAttribute("abasType").getName(),
									is(not("")));
							assertThat("field tag has value", field.getValue(), is(not("")));
						}
					}
